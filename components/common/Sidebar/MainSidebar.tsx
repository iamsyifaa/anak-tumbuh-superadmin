...
// Navigasi utama Super Admin: dashboard, master sekolah, dan akun kepala
// sekolah. Cuma tampil di layar >= lg (desktop); di mobile diganti
// MainBottomNav (lihat components/common/BottomNav).
const NAV_ITEMS = [
...
function MainSidebar({ onClickLogout }: Props) {
  const pathname = usePathname();
  const { user } = useSelector((state: RootState) => state.auth);

  return (
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-60 flex-col bg-[#203A5B] lg:flex">
